...
void Hero::animation_move_to_up(Ref * sender, ui::Widget::TouchEventType type)//обработчик дл€ кнопки движени€ вверх
{
	switch (type)
	{
	case ui::Widget::TouchEventType::BEGAN: {
		...
		// ƒвигает спрайт на 50 пикселей вверх за 2 секунды.
		MoveBy* moveByUp = MoveBy::create(0.2, Vec2(0, 50)); //вверх
		Spawn* spawn = Spawn::create(moveByUp, nullptr); // Spawn - действи€ выполн€емые параллельно, цепочка заканчиваетс€ nullptr
		this->runAction(spawn); //запускаем движение дл€ персонажа
		this->getChildByName("sprite")->runAction(Repeat::create(animate, 1)); //запуск анимации дл€ спрайта
		break;
	}
